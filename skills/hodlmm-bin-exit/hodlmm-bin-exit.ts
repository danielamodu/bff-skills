...
program
  ...
  .action(async (opts: { poolId: string }) => {
    ...
    // API reachability
    try {
      await fetchJson(`${BITFLOW_API}/hodlmm/pools/${opts.poolId}`);
    ...
    try {
      ...
      checks["pool"] = {
        ok: true,
        poolId: opts.poolId,
        tokenX: (pool as HodlmmPoolInfo).token_x_symbol || (pool as HodlmmPoolInfo).token_x,
        tokenY: (pool as HodlmmPoolInfo).token_y_symbol || (pool as HodlmmPoolInfo).token_y,
      };
    } catch (e: unknown) {
      const msg = e instanceof Error ? e.message : String(e);
